...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Videostich":
		...
		case "ContainerEcho":
			...
			return (*Videostich).ContainerEcho(&parent, stringArg), nil
		case "GrepDir":
			var err error
			var parent Videostich
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Videostich).GrepDir(&parent, ctx, ptr(directoryArg), pattern)
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Videostich").
					WithFunction(
